...
class AddAppointmentsRescheduledHistory(object):
    @staticmethod
    def initialize():
    ...
    @staticmethod
    def add_appointment_reschedule_history():
        reschedule_history_operations = RescheduleHistoryOperations()
        request = BodyWrapper()
        data = []
        reschedule_history = RescheduleHistory()
        appointment_name = AppointmentName()
        appointment_name.set_name("Test")
        appointment_name.set_id(440248001501005)
        reschedule_history.set_appointment_name(appointment_name)
        rescheduled_by = User()
        rescheduled_by.set_id(4402480254001)
        rescheduled_by.set_name("username")
        reschedule_history.set_rescheduled_by(rescheduled_by)
        reschedule_history.set_rescheduled_to(datetime.datetime(2023, 8, 30, 16, 30, 1))
        reschedule_history.set_rescheduled_from(datetime.datetime(2023, 8, 30, 13, 0, 0))
        reschedule_history.set_rescheduled_time(datetime.datetime(2023, 8, 26, 16, 30, 1))
...
AddAppointmentsRescheduledHistory.add_appointment_reschedule_history()