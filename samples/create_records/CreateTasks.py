...
from zohocrmsdk.src.com.zoho.crm.api.record import RecordOperations, BodyWrapper, Record, Field, Consent, FileDetails, \
    CreateRecordsHeader, APIException, SuccessResponse, ActionWrapper, RemindAt, RecurringActivity
from zohocrmsdk.src.com.zoho.crm.api.tags import Tag
from zohocrmsdk.src.com.zoho.crm.api.users import MinifiedUser
from zohocrmsdk.src.com.zoho.crm.api.util import Choice, Utility
...
class CreateTasks(object):
    ...
    @staticmethod
    def create_tasks(module):
        record_operations = RecordOperations()
        body_wrapper = BodyWrapper()
        records = []
        record1 = Record()
        record1.add_field_value(Field.Tasks.description(), "Test Task")
        record1.add_key_value("Rupee", Choice("INR"))
        remindAt = RemindAt()
        remindAt.set_alarm("ACTION=EMAILANDPOPUP;TRIGGER=DATE-TIME:2023-07-03T12:30:00+05:30")
        record1.add_field_value(Field.Tasks.remind_at(), remindAt)
        whoId = Record()
        whoId.set_id(440248001541026)
        record1.add_field_value(Field.Tasks.who_id(), whoId)
        record1.add_field_value(Field.Tasks.status(), Choice("Not Started"))
        record1.add_field_value(Field.Tasks.due_date(), datetime.date(2023, 12, 13))
        record1.add_field_value(Field.Tasks.priority(), Choice("Low"))
        record1.add_field_value(Field.Tasks.subject(), "Email1")
        record1.add_field_value(Field.Tasks.send_notification_email(), False)
        record1.add_key_value("$se_module", "Accounts")
        whatId = Record()
        whatId.set_id(440248001538033)
        record1.add_field_value(Field.Tasks.what_id(), whatId)
        # Recurring Activity can be provided in any activity module
        recurringActivity = RecurringActivity()
        recurringActivity.set_rrule("FREQ=DAILY;INTERVAL=10;UNTIL=2023-08-14;DTSTART=2023-07-03")
        record1.add_field_value(Field.Events.recurring_activity(), recurringActivity)
...
CreateTasks.create_tasks(module)