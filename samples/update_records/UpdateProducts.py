...
from zohocrmsdk.src.com.zoho.crm.api.record import RecordOperations, BodyWrapper, Record, Field, Consent, FileDetails, \
    CreateRecordsHeader, APIException, SuccessResponse, ActionWrapper, Tax
...
class UpdateProducts(object):
    ...
    @staticmethod
    def update_products(module):
        record_operations = RecordOperations()
        body_wrapper = BodyWrapper()
        records = []
        record1 = Record()
        record1.set_id(323213213123)
        record1.add_field_value(Field.Products.product_name(), "product_name")
        record1.add_field_value(Field.Products.product_code(), "product_code")
        record1.add_field_value(Field.Products.product_active(), True)
        record1.add_field_value(Field.Products.product_category(), Choice("Software"))
        record1.add_field_value(Field.Products.sales_end_date(), datetime.date(2023, 12, 25))
        record1.add_field_value(Field.Products.support_expiry_date(), datetime.date(2025, 12, 12))
        record1.add_field_value(Field.Products.sales_start_date(), datetime.date(2023, 6, 12))
        record1.add_field_value(Field.Products.support_start_date(), datetime.date(2023, 6, 20))
        record1.add_field_value(Field.Products.manufacturer(), Choice("MetBeat Corp."))
        vendor = Record()
        vendor.add_field_value(Field.Vendors.id, 4402481054948)
        record1.add_field_value(Field.Products.vendor_name(), vendor)
        record1.add_field_value(Field.Products.description(), "description of product")
        # price info of Product
        record1.add_field_value(Field.Products.unit_price(), 33.22)
        record1.add_field_value(Field.Products.commission_rate(), 12.2)
        record1.add_field_value(Field.Products.taxable(), True)
        taxes = []
        tax = Tax()
        tax.set_id(440248020807)
        tax.set_value("15.0")
        record1.add_key_value("Tax", taxes)
        # stock info of Product
        record1.add_field_value(Field.Products.usage_unit(), Choice("Box"))
        record1.add_field_value(Field.Products.qty_in_stock(), 10.0)
        record1.add_field_value(Field.Products.qty_in_demand(), 5.0)
        record1.add_field_value(Field.Products.qty_ordered(), 5.0)
        record1.add_field_value(Field.Products.reorder_level(), 0.0)
        user = MinifiedUser()
        user.set_id(440248254001)
        record1.add_key_value("Handler", user)
...
UpdateProducts.update_products(module)