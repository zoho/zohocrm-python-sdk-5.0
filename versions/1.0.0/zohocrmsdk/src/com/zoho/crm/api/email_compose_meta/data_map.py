...
class DataMap(object):
	...
	def get_default_from(self):
		"""
		The method to get the default_from

		Returns:
			DefaultForm: An instance of DefaultForm
		"""

		return self.__default_from

	def set_default_from(self, default_from):
		"""
		The method to set the value to default_from

		Parameters:
			default_from (DefaultForm) : An instance of DefaultForm
		"""
		...
		if default_from is not None and not isinstance(default_from, DefaultForm):
			raise SDKException(Constants.DATA_TYPE_ERROR, 'KEY: default_from EXPECTED TYPE: DefaultForm', None, None)
		
		self.__default_from = default_from
		self.__key_modified['default_from'] = 1
